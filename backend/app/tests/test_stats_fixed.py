...
def test_stats_summary_with_date_range():
    ...
    response = client.get(f"/api/v1/stats/summary?from={from_date}&to={to_date}")
    assert response.status_code == 200
...
def test_stats_summary_invalid_date_format():
    """Test stats summary with invalid date format"""
    from app.core.auth import require_auth
    app.dependency_overrides[require_auth] = mock_auth_dependency
    
    response = client.get("/api/v1/stats/summary?from=invalid-date")
    assert response.status_code == 400
    assert "Invalid from date format" in response.json()["detail"]


def test_stats_summary_invalid_date_range():
...
def test_stats_export_domain():
    """Test domain stats CSV export"""
    from app.core.auth import require_auth
    app.dependency_overrides[require_auth] = mock_auth_dependency
    
    response = client.get("/api/v1/stats/export?scope=domain")
    assert response.status_code == 200
...
def test_stats_export_campaign():
    """Test campaign stats CSV export"""
    from app.core.auth import require_auth
    app.dependency_overrides[require_auth] = mock_auth_dependency
    
    response = client.get("/api/v1/stats/export?scope=campaign")
    assert response.status_code == 200
...
def test_stats_export_invalid_scope():
    """Test stats export with invalid scope"""
    from app.core.auth import require_auth
    app.dependency_overrides[require_auth] = mock_auth_dependency
    
    response = client.get("/api/v1/stats/export?scope=invalid")
...
def test_domain_stats_endpoint():
    ...
    response = client.get("/api/v1/stats/domains")
    assert response.status_code == 200
...
def test_campaign_stats_endpoint():
    """Test optional campaign stats endpoint"""
    from app.core.auth import require_auth
    app.dependency_overrides[require_auth] = mock_auth_dependency
    
    response = client.get("/api/v1/stats/campaigns")
    assert response.status_code == 200
...
# Service-level tests (no auth needed)
